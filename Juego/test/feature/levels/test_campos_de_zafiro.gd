...
func before_test() -> void:
	level = auto_free(load("res://scenes/CamposDeZafiro.tscn").instantiate())
	add_child(level)
	await get_tree().process_frame


func test_ready_sets_game_state() -> void:
	assert_int(GameState.current_level).is_equal(1)
	assert_str(GameState.current_level_path).is_equal("res://scenes/CamposDeZafiro.tscn")
...
func test_mover_player_fresh_spawn() -> void:
